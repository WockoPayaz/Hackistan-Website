...
export function useLogoTransition(ref: RefObject<HTMLDivElement | null>) {
  useLayoutEffect(() => {
      ...
      const timeline = gsap.timeline({
        ...
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            if (stage) stage.style.pointerEvents = self.progress > 0.6 ? "none" : "";
          },
        },
      });
      timeline
    ...
    const hashJump = () => {
      if (location.hash === "#now" || location.hash === "#top" || location.hash === "#upcoming" || location.hash === "#join") scrollToSection(location.hash.slice(1), true);
    };
    const anchorClick = (event: MouseEvent) => {
      if (event.defaultPrevented || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;
      const target = event.target;
      if (!(target instanceof Element)) return;
      const anchor = target.closest<HTMLAnchorElement>('a[href="#now"], a[href="#top"], a[href="#upcoming"], a[href="#join"]');
      if (!anchor) return;
